...
'<p>Use the <strong>FOS - Download File(s)</strong> plug-in to start the browser download of files stored in the database. The files can be compiled via a SQL query or procedurally in a PL/SQL code block. If multiple files are returned, they will be z'
||'ipped automatically. A single file can optionally be zipped as well. The files can be BLOBs or CLOBs.</p>',
